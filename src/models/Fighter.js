...
class Fighter extends Sprite {
  ...
  changeAnimationState(name) {
    if (
      this.extractedString === this.animations["attack"].imageSrc &&
      this.frameCurrent < this.animations["attack"].framesMax - 1
    ) {
      return;
    }
  ...
  update(enemiesToHandle) {
    ...
    if (this.isFlipped)
      ...
      this.attackBox.position.x =
        this.position.x + this.hitbox.w + this.attackBox.offset.x;

    this.attackBox.position.y = this.position.y + this.attackBox.offset.y;
  }
  autoFlipSprite() {
    if (this.velocity.x > 0 && this.isFlipped != false) this.flipSpriteRight();
