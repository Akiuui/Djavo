...
class Enemy extends Fighter {
  ...
  update(player, collisionForEnemies) {
    ...
    if (this.health < this.maxHealth && this.health > 0) this.drawHealthBar();

    if (this.health <= 0) {
      //Death animation
      this.isAlive = false;
      gameState.enemiesToUpdate = gameState.enemiesToUpdate.filter(
        (e) => e.isAlive,
      );
  ...
  drawHealthBar() {
    ...
    c.strokeRect(
      this.position.x - 10,
      this.position.y + this.hitbox.h + 5,
      this.hitbox.w + 20,
      7,
    );
    c.fillRect(
      this.position.x - 10,
      this.position.y + this.hitbox.h + 5,
      (this.hitbox.w + 20) * (this.health / this.maxHealth),
      7,
    );
  }
  movement(player) {
