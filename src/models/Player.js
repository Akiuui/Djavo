...
class Player extends Fighter {
  ...
  update(keys, enemiesToHandle) {
    ...
    if (this.isAttacking && this.frameCurrent == 3) this.isAttacking = false;
  ...
  movement(keys) {
    MovementOnX({
      ...
      left: "ArrowLeft",
      right: "ArrowRight",
      speedOfRunning: this.speedOfRunning,
    });
    MovementOnY({
      keys,
      player: this,
  ...
  drawXpBar() {
    document.getElementById("playerXp").style.width =
      (this.Xp / this.XpToLevelUp) * 100 + "%";
  }
  levelUp() {
  ...
  increaseSpeed() {
    this.speedOfRunning =
      this.speedOfRunning + this.speedOfRunning * (15 / 100);
  }
  increaseDamage() {
