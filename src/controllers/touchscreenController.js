...
function touchScreenController() {
  ...
  hitButton.addEventListener("touchstart", (e) => {
    if (!autoAttackToggled) keys.Space.pressed = true;
  });
  hitButton.addEventListener("touchend", (e) => {
    if (!autoAttackToggled) keys.Space.pressed = false;
  });
  ...
  joystick.addEventListener("touchstart", (event) => {
    ...
    joystickContainer.style.left =
      touchStartX - joystickContainer.offsetWidth / 2 + "px";
    joystickContainer.style.top =
      touchStartY - joystickContainer.offsetHeight / 2 + "px";
  });
  ...
  joystick.addEventListener("touchmove", (event) => {
    ...
    const containerCenterX =
      joystickContainer.offsetLeft + joystickContainer.offsetWidth / 2;
    const containerCenterY =
      joystickContainer.offsetTop + joystickContainer.offsetHeight / 2;
