...
function gameController(enemiesToUpdate, seconds) {
  if (
    NumberOfDefined(enemiesToUpdate) < gameState.maxEnemiesOnScreen &&
    seconds % 10 === 0 /* && activated === true*/
  ) {
    // activated = false
    let enemiesToAdd =
      gameState.maxEnemiesOnScreen - NumberOfDefined(enemiesToUpdate);
  ...
  if (seconds === 55) {
    gameState.gameState.namesOfEnemies = [
      ...gameState.namesOfEnemies,
      "nightmare",
    ];
  }
  if (seconds === 75 /* && act1*/) {
