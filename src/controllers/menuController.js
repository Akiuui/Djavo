...
export function SaveScore() {
  ...
  if (localStorage.getItem(input) === null) {
    ...
    let result = confirm(
      "Score with that name is already saved, Do you want to overwrite it?",
    );
    if (result) {
      document.getElementById("form").style.display = "none";
      document.getElementById("form1").style.display = "none";
...
export function muteMusic() {
  ...
  if (now) {
    ...
    document.getElementById("muteButton").innerHTML =
      '<svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 24 24" fill="none" stroke="#474747" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 5L6 9H2v6h4l5 4zM22 9l-6 6M16 9l6 6"/></svg>';
    now = false;
  } else {
    audio.play();
    document.getElementById("muteButton").innerHTML =
